    ...
    <el-table :data="departments" highlight-current-row v-loading="listLoading" @selection-change="selsChange"
      ...
      <el-table-column label="操作">
        <template scope="scope">
          ...
          <el-button v-perm="'department:delete'" type="danger" size="small"
...
export default {
  ...
  methods: {
     ...
     */
    queryAllEmp() {
      this.$http.get("/org/employee/queryAll")
          .then(res => {
            if (res.data.success) {
    ...
    getDepartments() {
      ...
      this.$http.post("/org/dept/queryPage", para).then(res => {
        ...
        this.departments = res.data.data.list;//分页后，返回就是当前页的list数据
        //3.总条数赋值
    ...
    handleDel: function (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        axios.delete("/org/dept/delete/" + row.id).then(res => {
          if (res.data.success) {
            this.$message({
              message: '删除成功',
              type: 'success'
    ...
    batchRemove: function () {
      var ids = this.sels.map(item => item.id); // Keep as array, e.g., [1, 2, 3]
      console.log(ids);
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        //this.listLoading = true;
        //NProgress.start();
        axios.delete("/org/dept/batchremove", {
          data: ids // Axios will JSON.stringify this array automatically
        }).then(res => {
          if (res.data.success) {
            this.$message({
              message: '删除成功',
    ...
    /*batchRemove: function () {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        //NProgress.start();
        let para = {ids: ids};
        axios.delete("/org/dept/batchremove",JSON.stringify(ids)).then(res => {
          if (res.data.success) {
            this.$message({
              message: '删除成功',
              type: 'success'
