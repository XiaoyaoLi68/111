...
<!--        <el-form-item>
    ...
    <el-table :data="pageData.list" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
...
export default {
  ...
  methods: {
    ...
    getSearchMasterMsgs() {
      ...
      this.$http.post("/searchMaster/searchMasterMsg/toSearchMasterPool",this.query)
          .then(result => {
    ...
    handleAccept: function (index, row) {
      this.$confirm('确认接单吗?', '温馨提示', {
        type: 'warning'
      }).then(() => {
        this.listLoading = true;
        let url = "/searchMaster/searchMasterMsg/accept";
        this.$http.post(url, {msgId: row.id})
            .then((result) => {
              result = result.data; //{success:true,message:"xx"}
