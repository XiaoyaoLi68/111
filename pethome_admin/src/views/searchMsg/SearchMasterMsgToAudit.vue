<template>
  <section>
    ...
    <el-table :data="pageData.list" highlight-current-row v-loading="listLoading" @selection-change="selsChange"
      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          <el-table :data="props.row.audit" style="width: calc(100% - 47px)" class="two-list">
      ...
      <el-table-column prop="note" label="审核备注" width="120" sortable>
      </el-table-column>
    ...
    <!--审核界面-->
    <el-dialog title="审核" :visible.sync="auditFormVisible" :close-on-click-modal="false">
      <el-form :model="audit" label-width="80px" ref="auditForm">
        ...
        <el-form-item label="备注">
          <el-input v-model="audit.note" type="textarea" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click.native="auditFormVisible = false">取消</el-button>
        <el-button type="primary" @click.native="submitAudit" :loading="auditLoading">提交</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getSearchMasterMsgs() {
      ...
      this.$http.post("/searchMaster/searchMasterMsg/toAudit", this.query)
          .then(result => {
    ...
    handleAudit: function (index, row) {
      ...
      this.audit.searchMasterMsgId = row.id;
      this.audit.state = 1;
      this.audit.note = '';
    },
    submitAudit: function () {
      this.$confirm('确认提交审核吗？', '提示', {}).then(() => {
        ...
        let para = Object.assign({}, this.audit);
        this.$http.post("/searchMaster/searchMasterMsg/audit", para)
            .then((result) => {
